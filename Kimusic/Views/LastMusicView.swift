...
struct LastMusicView: View {
    ...
    var body: some View {
        VStack{
            ...
            Task{
                lastMusic = MusicModel(idCode: lastMusic.idCode, MusicTitle: lastMusic.MusicTitle, MusicBanner: lastMusic.MusicBanner, ArtistName: lastMusic.ArtistName, showTab: true)
                if let data = try? JSONEncoder().encode(MusicModel(idCode: lastMusic.idCode, MusicTitle: lastMusic.MusicTitle, MusicBanner: lastMusic.MusicBanner, ArtistName: lastMusic.ArtistName, showTab: true)){
                    UserDefaults.standard.set(data, forKey: "lastMusic")
                }
                await vmtask.executedMusic(idCode: lastMusic.idCode, titleMusic: lastMusic.MusicTitle, MusicImage: lastMusic.MusicBanner, MusicArtist: lastMusic.ArtistName)
            }
        }
    ...
    private var MusicHeader : some View{
        HStack{
            VStack(alignment: .leading, spacing: 8) {
                Text(lastMusic.MusicTitle)
                    .fontWeight(.semibold)
                
                HStack(spacing: 10){
                    
                    Text(lastMusic.ArtistName)
                        .font(.caption)
                }
            }
